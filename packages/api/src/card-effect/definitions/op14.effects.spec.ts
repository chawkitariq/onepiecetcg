...
class TestHost implements EffectEngineHost {
  ...
  public addPlayer(sessionId: string, leaderCardId = `L-${sessionId}`): DuelPlayer {
    ...
    player.zones.leader = createDuelCard(
      makeCard({ id: leaderCardId, number: leaderCardId, name: `${sessionId} Leader`, type: 'Leader', power: 5000, life: 5 }),
  ...
  public addLog(message: string): void { this.logs.push(message); }
  ...
  public getPlayer(sessionId: string): DuelPlayer | undefined { return this.state.players.get(sessionId); }
  ...
  public getCards(selector: any, controllerSessionId: string): DuelCard[] {
    const sessionIds = selector.player === 'self' ? [controllerSessionId]
      : selector.player === 'opponent' ? [this.getOpponentSessionId(controllerSessionId)].filter(Boolean)
    ...
    for (const sessionId of sessionIds) {
      ...
      for (const zone of selector.zones) {
        const cards = zone === 'leader' ? [player.zones.leader]
          : zone === 'stage' ? (player.zones.stage.instanceId ? [player.zones.stage] : [])
        ...
        for (const card of cards) {
          ...
          if (selector.filter?.basePowerMax != null && card.basePower > selector.filter.basePowerMax) continue;
          if (selector.filter?.baseCostMax != null && card.baseCost > selector.filter.baseCostMax) continue;
          ...
          if (selector.filter?.owner === 'self' && card.ownerSessionId !== controllerSessionId) continue;
          if (selector.filter?.owner === 'opponent' && card.ownerSessionId === controllerSessionId) continue;
          matches.push(card);
        }
      }
    }
  ...
  public moveCard(card: DuelCard, destPlayerSessionId: string, destZone: string, opts?: any): void {
    ...
    if (destZone === 'trash') { player.zones.trash.unshift(card); }
    else if (destZone === 'hand') { player.zones.hand.push(card); }
    else if (destZone === 'life' && opts?.toBottom) { player.zones.life.push(card); }
    else if (destZone === 'life') { player.zones.life.unshift(card); }
    else if (destZone === 'deck') { player.zones.deck.push(card); }
    else if (destZone === 'donDeck') { player.zones.donDeck.push(card); }
    else if (destZone === 'cost') { player.zones.cost.push(card); }
    else if (destZone === 'characters') { player.zones.characters.push(card); }
    else if (destZone === 'stage') { player.zones.stage = card; }
  }
  ...
  public attachDon(playerSessionId: string, targetInstanceId: string, amount: number, opts?: any): number {
    const player = this.getPlayer(playerSessionId);
    const target = player?.zones.leader.instanceId === targetInstanceId ? player.zones.leader
      : player?.zones.characters.find((c) => c.instanceId === targetInstanceId);
    if (!player || !target) return 0;
    const matchingDon = player.zones.cost.filter((c) => opts?.rested === undefined ? true : c.rested === opts.rested);
  ...
  public addCardToZone(playerSessionId: string, zone: string, card: Card, instanceSuffix: string): DuelCard {
    const duelCard = createDuelCard(card, `${playerSessionId}:${instanceSuffix}`, playerSessionId);
    const z = zone as 'hand' | 'deck' | 'donDeck' | 'characters' | 'trash' | 'cost' | 'life';
...
describe('op14EffectDefinitions', () => {
  ...
  it('OP14-002 urouge-when-attacking draws and KOs when power >= 5000', () => {
    ...
    const urouge = host.addCardToZone('p1', 'characters', makeCard({
      id: 'OP14-002', number: 'OP14-002', name: 'Urouge', type: 'Character', power: 5000,
    }), 'urouge');
    host.addCardToZone('p2', 'characters', makeCard({
      id: 'TARGET', number: 'TARGET', name: 'Target', type: 'Character', power: 3000,
    }), 'target');
    host.addCardToZone('p1', 'deck', makeCard({ id: 'DECK1', number: 'DECK1', name: 'Draw1', type: 'Character' }), 'deck1');

    const registry = createRegistry();
    expect(registry.effectsByCardId['OP14-002']?.standard).toBeDefined();
    expect(registry.effectsByCardId['OP14-002']!.standard![0].id).toBe('urouge-when-attacking-5000-draw-ko-3000');
  });

  it('OP14-032 effect is registered via special-ref', () => {
    const registry = createRegistry();
    expect(registry.effectsByCardId['OP14-032']?.specialHandlerId).toBe('op14-032-special');
  });
  ...
  it('OP14-092 mr-3-replacement registers a replacement effect for wouldKoCharacter', () => {
    ...
    expect(registry.effectsByCardId['OP14-092']!.replacements![0].id).toBe('mr-3-opponent-turn-ko-replacement');
  });
  ...
  it('OP14-108 silvers-rayleigh-on-play registers both onPlay and trigger effects', () => {
    ...
    expect(effects!.standard![0].id).toBe('silvers-rayleigh-on-play-multicolor-ko-7000');
    expect(effects!.standard![1].id).toBe('silvers-rayleigh-trigger-activate-on-play');
  });
  ...
  it('continuous effect OP14-004 registers correctly', () => {
    ...
    expect(effects!.continuous![0].conditions![0]).toEqual({ type: 'sourcePowerAtLeast', value: 5000 });
  });

  it('all special handlers for OP14 are registered in the index', () => {
    const handlerIds = specialHandlerDefinitions.map(h => h.id);
    const expected = [
      'op14-001-special', 'op14-009-special', 'op14-011-special',
      'op14-016-special', 'op14-017-special', 'op14-020-special',
      'op14-021-special', 'op14-027-special', 'op14-028-special',
      'op14-032-special', 'op14-033-special', 'op14-034-special',
      'op14-035-special', 'op14-041-special', 'op14-044-special',
      'op14-045-special', 'op14-049-special', 'op14-053-special',
      'op14-056-special', 'op14-060-special', 'op14-061-special',
      'op14-062-special', 'op14-069-special', 'op14-070-special',
      'op14-079-special', 'op14-096-special', 'op14-103-special',
      'op14-104-special', 'op14-105-special', 'op14-111-special',
      'op14-115-special', 'op14-119-special',
    ];
