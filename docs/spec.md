...
- 🎯 **Google et Discord en production** ; en **développement local uniquement**, un accès Better Auth **anonyme** est activé pour les tests contributeur. Décision actée : aucun login email/mot de passe n'est prévu.
...
- **Déclenchement de l'enregistrement** : un résultat de partie est persisté sur une fin de partie propre au sens du moteur structurel (§3) — Vie à zéro, deck-out, ou **abandon explicite** (un joueur quitte volontairement la partie en cours, ex: bouton "Retourner au lobby" du plateau) — dans tous les cas via `DuelState.phase === 'finished'` avec un vainqueur déterminé. Un abandon par **déconnexion** après le délai de reconnexion (§3, 2 minutes) reste le seul cas qui ne génère **aucun** enregistrement de statistique — le joueur n'a pas choisi de partir, une coupure réseau ne doit pas être pénalisée comme un vrai abandon ; seule la défaite structurelle immédiate déjà prévue pour la room en cours s'applique, sans trace persistée au-delà.
- **`DuelState` doit exposer explicitement le vainqueur** : la room passe en `phase: 'finished'` avec un champ structuré (`winnerSessionId`, motif de fin `life`/`deckOut`/`forfeit`) exploitable par le serveur pour savoir qui a gagné sans reparser un log.
- **Un résultat de partie enregistré** conserve : les deux comptes (vainqueur/perdant), les decks utilisés par chaque joueur (référence nullable — un deck supprimé ensuite ne doit pas faire disparaître le résultat), le **Leader utilisé par chaque joueur au moment de la partie** (conservé indépendamment du deck, pour rester exploitable même si le deck est modifié ou supprimé par la suite), qui jouait en premier, le motif de fin (`life`/`deckOut`/`forfeit`) et la durée de la partie. Un abandon explicite compte comme une défaite à part entière pour l'abandonnant dans toutes les statistiques (global, par deck, par Leader, série en cours) — un joueur ne doit pas pouvoir esquiver une partie mal engagée sans que cela affecte son registre.
